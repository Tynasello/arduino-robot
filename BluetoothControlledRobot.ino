/*  Ty Nasello
 *  05-8-2020
 *  Robot Program using input from bluetooth device and optional autonomous mode
 */
...
void loop() {
  ...
  if (bluetoothSerial.available()){//if data is available for reading from serial port 
...
void autonomous(){ //autonomous mode
  ...
  while(true){
    ...
    if (objDistance<=objClose && objDistance != 0){//if in range of object that is to close (within 10 cm of)
    ...
    }else{//if no objects are very close
      goForward();
      in = bluetoothSerial.read();//read in character incase we want to quit autonomous mode
    }
    if(in == 'q'){
